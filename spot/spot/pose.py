...
from spot.walk.open_loop_poses_env import Poses
import numpy as np
import time
...
class StartPoses:
    def __init__(self,):
        ...
        self.shoulder_inter = interp1d([-3.142, 3.142], [0, 180])
        self.elbow_inter = interp1d([-math.pi/2, math.pi/2], [0, 180])
        self.left_wrist_inter = interp1d([-0.2, math.pi], [0, 180])

        ####################################################

        base_x = {'min':-0.02 , 'max': 0.02 , 'default': 0.010,}
        base_y = {'min': -0.007, 'max': 0.007, 'default': 0,}
        base_z = {'min': -0.048, 'max': 0.021, 'default': 0 ,}

        roll = {'min': -0.785, 'max': 0.785 , 'default': 0,}
        pitch = {'min': -0.785, 'max': 0.785 , 'default': 0,}
        yaw = {'min': -0.785, 'max': 0.785, 'default': 0,}
    ...
    def start_walking(self, motor_values: np.array):
        ...
        for idx, val in enumerate(motor_values):
            if idx in [2,8]:
                ...
                self.kit.servo[self.irl_spot[self.sim_spot[idx]]].angle = int(self.shoulder_inter(val))

            elif idx in [9]:
                self.kit.servo[self.irl_spot[self.sim_spot[idx]]].angle = 180 - int(self.shoulder_inter(val))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for _ in range(20):
            ...
            actions = pose_helper._signal(current_time, temp_actions, pos, ori)
